...
; TeamViewer supports /S for silent install
...
Func _IsTeamViewerInstalled()
    Local $sPath = RegRead("HKLM64\SOFTWARE\TeamViewer", "InstallationDirectory")
    If Not @error And FileExists($sPath & "\TeamViewer.exe") Then Return True
    Local $sPath32 = RegRead("HKLM\SOFTWARE\TeamViewer", "InstallationDirectory")
    If Not @error And FileExists($sPath32 & "\TeamViewer.exe") Then Return True
    Return FileExists(@ProgramFilesDir & "\TeamViewer\TeamViewer.exe")
EndFunc
...
Func _CreateDesktopShortcut()
    ...
    Local $sTarget = @ProgramFilesDir & "\TeamViewer\TeamViewer.exe"
    Local $sReg = RegRead("HKLM64\SOFTWARE\TeamViewer", "InstallationDirectory")
    If Not @error And $sReg <> "" Then $sTarget = $sReg & "\TeamViewer.exe"
